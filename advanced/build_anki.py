#!/usr/bin/env python3
"""Build an Anki .apkg study deck for the French ADVANCED course (C1-C2).

Sources:
  - vocab-data.js  (window.FRENCH_VOCAB) — the per-lesson review vocabulary.
  - cheatsheet.js  (SHEET) — the mastery cheat sheet.
...
Output: french_advanced.apkg

IDs are distinct from the beginner and intermediate decks' so the three decks coexist in Anki
...
def main():
    ...
    model = genanki.Model(
        MODEL_ID,
        "French Advanced (French/English)",
        fields=[
            {"name": "French"},
            {"name": "Pronunciation"},
            {"name": "English"},
...
""",
    ...
    deck = genanki.Deck(DECK_ID, "French Course (C1-C2 Advanced)")
    ...
    out = os.path.join(HERE, "french_advanced.apkg")
