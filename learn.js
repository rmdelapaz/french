/* learn.js — learning-experience layer for the French course.
...
(function () {
    ...
    var NS = IS_ADV ? 'french-adv-' : (IS_INT ? 'french-int-' : 'french-');
    ...
    var VOCAB = window.FRENCH_VOCAB || {};
    var TITLES = (VOCAB.titles) || {};
    var SLUGS = (VOCAB.slugs) || {};   // { "1": "french_greetings_essentials", ... } (beginner tier)
    var LESSON_INFO = window.FRENCH_LESSON_INFO || {};
    ...
    /* ---------- page identity ----------
       Beginner pages are topic-named (french_greetings_essentials.html) and are
       mapped to a lesson number via VOCAB.slugs. Intermediate pages are numbered
       (french_lesson_N.html). Either way we resolve a lessonNum string. */
    ...
    if (!lessonNum) { var lessonMatch = page.match(/french_lesson_(\d+)/); lessonNum = lessonMatch ? lessonMatch[1] : null; }
    ...
    function lessonHref(n) {
        ...
        return TIER_BASE + 'french_lesson_' + n + '.html';
    }
    ...
    function vocabImg(w, alt) {
        ...
        return '<img class="lx-word-img" src="' + src + '" alt="' + esc(alt || w.en || w.fr) +
    ...
    /* ---------- spaced-repetition store (Leitner boxes) ----------
       french-srs: { "<es>": { box: 1..5, due: <ts>, reps: <n> } }
       ...
       occasional. The older flat french-known map is migrated in on first read. */
    var DAY = 86400000;
    ...
    function dueFrom(words) {
        ...
        words.forEach(function (w) { var e = m[w.fr]; if (e && e.due <= now) out.push(w); });
        return out;
    }
    function trackedFrom(words) {
        var m = srsMap(), seen = {}, out = [];
        words.forEach(function (w) { if (m[w.fr] && !seen[w.fr]) { seen[w.fr] = 1; out.push(w); } });
        return out;
    }
    function flatVocab() {
        var all = [], seen = {};
        for (var ln in VOCAB) {
            ...
            VOCAB[ln].forEach(function (w) { if (!seen[w.fr]) { seen[w.fr] = 1; all.push(w); } });
        }
        return all;
    }
    ...
    function initObjectives() {
        ...
        sec.innerHTML =
            ...
            (info.time ? '<p class="lx-meta">⏱️ About ' + esc(info.time) + ' · ' +
                (info.level ? 'CEFR ' + esc(info.level) : 'Beginner') + '</p>' : '');
    ...
    function initVocabReview() {
        ...
        function cardHTML(w) {
            var on = isTracked(w.fr);
            return '<div class="lx-word' + (on ? ' lx-known' : '') + (w.img ? ' lx-word-has-img' : '') + '" data-fr="' + esc(w.fr) + '">' +
                     vocabImg(w) +
                     '<div class="lx-word-tl"><span data-speak="' + esc(w.fr) + '">' + esc(w.fr) + '</span></div>' +
        ...
        function refresh() {
            var k = 0;
            list.forEach(function (w) { if (isTracked(w.fr)) k++; });
        ...
        sec.addEventListener('click', function (e) {
            ...
            var tl = card.getAttribute('data-fr');
            var on = !isTracked(tl);
            trackWord(tl, on);
        ...
        markAllBtn.addEventListener('click', function () {
            var allOn = list.every(function (w) { return isTracked(w.fr); });
            list.forEach(function (w) { trackWord(w.fr, !allOn); });
            Array.prototype.forEach.call(sec.querySelectorAll('.lx-word'), function (card) {
                var on = isTracked(card.getAttribute('data-fr'));
                card.classList.toggle('lx-known', on);
        ...
        function renderCard() {
            ...
            if (!flipped) {
                faceEl.innerHTML = (w.img ? '<div class="lx-flash-img">' + vocabImg(w) + '</div>' : '') +
                    '<div class="lx-flash-front"><span data-speak="' + esc(w.fr) + '">' + esc(w.fr) + '</span></div>' +
    ...
    /* ==========================================================
       Self-check quiz  (French → English, auto-generated)
       ========================================================== */
    function initQuiz() {
        var list = VOCAB[lessonNum];
        ...
        function build() {
            ...
            body.innerHTML = picks.map(function (ans, qi) {
                ...
                return '<div class="lx-quiz-q" data-qi="' + qi + '">' +
                         '<p class="lx-quiz-stem">' + (qi + 1) + '. What does <span class="lx-q-word" data-speak="' + esc(ans.fr) + '">' + esc(ans.fr) + '</span> mean?</p>' +
    ...
    function initJournal() {
        ...
        sec.innerHTML =
            ...
            '<div class="lx-journal-prompts">Reflection helps it stick. Try: <strong>What clicked?</strong> · ' +
                ...
                '<strong>A sentence I can now say in French.</strong></div>' +
    ...
    function exportJournal() {
        ...
        function label(slug) {
            ...
            if (!num) { var m = slug.match(/french_lesson_(\d+)/); if (m) num = m[1]; }
            if (num) return 'Lesson ' + num + (TITLES[num] ? ' — ' + TITLES[num] : '');
            return slug;
        }
        var md = entries.length
            ? '# My French Learning Journal\n\n' + entries.map(function (e) {
                return '## ' + label(e.page) + '\n_saved ' + fmt(e.ts) + '_\n\n' + e.text + '\n';
              }).join('\n---\n\n')
        ...
        a.href = url; a.download = 'my-french-journal.md';
    ...
    function initDashboard() {
        ...
        Array.prototype.forEach.call(container.querySelectorAll('.lesson-card'), function (card) {
            ...
            if (!num) { var m = href.match(/french_lesson_(\d+)/); if (m) num = m[1]; }
            if (!num) return;
            var st = p[num] || {};
