/* glossary.js — builds the course-wide vocabulary glossary from vocab-data.js.
   ...
   deduplicated reference. Each unique French word appears once, linked back to
   ...
   lessons use (localStorage `french-srs`). No dependency on learn.js. */
(function () {
    'use strict';
    ...
    var K_SRS = /\/advanced\//.test(location.pathname) ? 'french-adv-srs'
              : /\/intermediate\//.test(location.pathname) ? 'french-int-srs' : 'french-srs';
    function lessonHref(n) { return SLUGS[n] ? SLUGS[n] + '.html' : 'french_lesson_' + n + '.html'; }
    ...
    LESSON_KEYS.forEach(function (ln) {
        ...
        list.forEach(function (w) {
            ...
            if (!byKey[key]) { byKey[key] = { fr: w.fr, pron: w.pron || '', en: w.en || '', lessons: [ln] }; order.push(key); }
    ...
    root.innerHTML =
        '<div class="g-controls">' +
            '<input type="search" id="g-search" class="g-search" placeholder="Search French or English…" aria-label="Search vocabulary" autocomplete="off">' +
            ...
            '<select id="g-sort" class="g-select" aria-label="Sort order">' +
                '<option value="tl">Sort: French A–Z</option>' +
        ...
        '<div class="table-wrap"><table class="g-table"><thead><tr>' +
            '<th>French</th><th>Pronunciation</th><th>English</th><th>Lesson</th><th class="g-th-act">Review</th>' +
    ...
    function rowHTML(w) {
        ...
        return '<tr data-fr="' + esc(w.fr) + '">' +
            '<td class="g-tl"><span data-speak="' + esc(w.fr) + '">' + esc(w.fr) + '</span></td>' +
    ...
    function currentFiltered() {
        ...
        var rows = WORDS.filter(function (w) {
            ...
            if (!q) return true;
            return fold(w.fr).indexOf(q) >= 0 || fold(w.en).indexOf(q) >= 0 || fold(w.pron).indexOf(q) >= 0;
        });
        ...
        rows.sort(function (a, b) {
            ...
            if (sort === 'lesson') {
                ...
                return d || a.fr.localeCompare(b.fr);
            }
            return fold(a.fr).localeCompare(fold(b.fr));
        });
        return rows;
    }
    ...
    // A–Z quick jump (active when sorted by French with no search)
    (function buildAZ() {
        var letters = {};
        WORDS.forEach(function (w) { var c = fold(w.fr).charAt(0).toUpperCase(); if (/[A-Z]/.test(c)) letters[c] = 1; });
    ...
    bodyEl.addEventListener('click', function (e) {
        ...
        if (!btn) return;
        var tl = btn.closest('tr').getAttribute('data-fr');
        var on = !isTracked(tl);
        trackWord(tl, on);
